    ...
    void command_thread_func() {
        while (!quit.load()) {
            std::string cmd;
            {
                ...
                queueCV.wait(lock, []{ return !commandQueue.empty() || quit.load(); });
...
void fs_init(const char* variantName) {
    ...
    // Ensure engine is ready
    {
        ...
        fs_command("isready");
        apiCV.wait_for(lock, std::chrono::seconds(2), []{ return true; });
    }
}
...
void fs_command(const char* cmd) {
    ...
    if (!cmd) return; // Avoid nullptr segfaults
...
void fs_set_option(const char* name, const char* value) {
    ...
    if (!name || !value) return; // Avoid nullptr segfaults
...
void fs_set_position(const char* fen) {
    ...
    if (!fen) return; // Avoid nullptr segfaults
...
void fs_set_position_after_moves(const char* fen, const char* moves) {
    ...
    if (!fen || !moves) return; // Avoid nullptr segfaults
...
const char* fs_get_best_move(int time_ms) {
    ...
    {
        std::unique_lock<std::mutex> lock(apiMutex);
        if (!apiCV.wait_for(lock, std::chrono::milliseconds(time_ms * 2), []{ return bestMoveReady.load(); })) {
